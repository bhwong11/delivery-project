...
router.post(`/:id/edit`, async (req, res) => {
  try {
    ...
    if (newCity) {
      ...
      await MessageBoard.updateOne(
        {
          $and: [{ users: { $in: [unupdatedUser._id] } }, { category: 'city' }],
        },
        { $pull: { users: { $elemMatch: { _id: unupdatedUser._id } } } },
        { new: true }
      );
    ...
    const updatedUser = await User.findByIdAndUpdate(req.userId, updateBody, {
      new: true,
    })
      .populate('messageBoards')
      .exec((err, messageBoards) => {
    ...
    if (!updatedUser) {
      return res.redirect('/index', {
        err: 'updated user was no able to be found',
      });
    }
